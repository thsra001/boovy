...
impl Plugin for PEditor {
    fn build(&self, app: &mut App) {
        app
            ...
            .add_systems(
                Update,
                toggle_state.run_if(input_just_pressed(KeyCode::KeyP)),
            )
            .add_plugins(FrameTimeDiagnosticsPlugin)
            .add_plugins(EditorCameraManager);
    }
}
fn toggle_state(state: Res<State<BoovyStates>>, mut next_state: ResMut<NextState<BoovyStates>>) {
    info!("state is {:?}", state.get());
    match state.get() {
        BoovyStates::Editor => next_state.set(BoovyStates::Game),
        BoovyStates::Game => next_state.set(BoovyStates::Editor),
        _ => panic!("wth dont press enter in other states"),
    };
}
...
//         window_state.set_size(egui_dock::egui::Vec2::splat(100.0));
//     });
// }

// every thing over this is fucked
